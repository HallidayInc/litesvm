...
export abstract class BaseTokenProgram {
    /** On-chain program address. Set by each concrete subclass. */
...
/** The original SPL Token program (`Tokenkeg…`). */
export class TokenProgram extends BaseTokenProgram {
    static override programId = new PublicKey(TOKEN_PROGRAM_ID);
}

/** The Token-2022 program (`Tokenz…`) — SPL Token plus the extensions model. */
export class Token2022Program extends BaseTokenProgram {
    static override programId = new PublicKey(TOKEN_2022_PROGRAM_ID);
}

export const sol_token_standard_to_program_id: Record<string, string> = {
    SPL: TokenProgram.programId.toBase58(),
    Token2022: Token2022Program.programId.toBase58(),
...
export class BpfLoaderUpgradeable {
    ...
    /**
     * Close an upgradeable-loader account, refunding its lamports to
     * `recipient`. Pass `program` to close a deployed program's ProgramData
     * (permanently retiring the program); omit it to close a standalone
     * buffer / uninitialized account. The current `authority` must sign — an
     * immutable program (authority already dropped) cannot be closed.
     */
    static close(
        account: PublicKey,
        recipient: PublicKey,
        ...
        const data = new Uint8Array(4);
        new DataView(data.buffer).setUint32(0, 5, true); // Close = 5
        const keys = [
            { pubkey: account, isSigner: false, isWritable: true },
            { pubkey: recipient, isSigner: false, isWritable: true },
            { pubkey: authority, isSigner: true, isWritable: false },
        ];
        // Closing a program (vs a bare buffer) also passes the program account,
        // which the loader marks closed.
        if (program) {
